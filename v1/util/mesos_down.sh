...
EOF
    if [ 0 -lt $(curl  -Ls -v -H "Content-type: application/json" -X POST -d @$tmpdir/down-host.json  ${MESOS_CREDS} ${MESOS_ELB}/machine/down 2>&1 | grep -c 'HTTP.*200.*OK') ] && \
	   [ ! -z "$( curl -SsL ${MESOS_CREDS} ${MESOS_ELB}/maintenance/status | jq --arg ip ${LOCAL_IP} --arg host $(hostname --fqdn) '.down_machines[]|select( .ip ==$ip  and .hostname==$host )')" ]; then
